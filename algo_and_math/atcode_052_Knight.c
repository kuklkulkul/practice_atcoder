...
int main(void)
{
    ...
    if ((inpt_X + inpt_Y) % 3U != 0U)
    {
        printf("0\n");
        return 0;
    }
    else if (((2U * inpt_X) < inpt_Y) || ((2U * inpt_Y) < inpt_X))
    {
        printf("0\n");
        return 0;
    }
    ...
    {
        x_ = (2U * inpt_X - inpt_Y) / 3U;
        y_ = (2U * inpt_Y - inpt_X) / 3U;
    }

    if (x_ >= y_)
    {
        ans = combination_mod((uint64_t)(x_ + y_), (uint64_t)y_, MOD_NUM);
    }
    else
    {
        ans = combination_mod((uint64_t)(x_ + y_), (uint64_t)x_, MOD_NUM);
    }
